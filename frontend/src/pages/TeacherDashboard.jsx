...
export default function TeacherDashboard() {
  ...
  return (
    <div className="fade-in">
      <h1 className="text-3xl font-bold text-white mb-2">Class Overview</h1>
      <p className="text-slate-400 mb-8">Monitor student progress, identify at-risk learners, and provide mentorship.</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="stat-card">
          ...
          <p className="text-3xl font-bold text-red-400">{weakCount}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-white/5">
                  {students.map(s => (
                    <tr key={s.id} className={`hover:bg-white/5 transition cursor-pointer ${s.isWeak ? 'bg-red-500/5' : ''}`} onClick={() => viewStudent(s)}>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-sm text-slate-400">{s.targetRole || '—'}</td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-400 font-semibold w-8">{s.progressPercent}%</span>
                        </div>
                      </td>
                      <td className="p-4">
                        <span className={`font-bold text-sm ${s.averageInterviewScore >= 7 ? 'text-emerald-400' : s.averageInterviewScore >= 4 ? 'text-amber-400' : 'text-red-400'}`}>
        ...
        <div className="space-y-6">
          {selectedStudent ? (
            <>
              ...
              <div className="glass-card p-6">
                ...
                <div className="grid grid-cols-2 gap-3 text-center">
                  <div className="p-3 rounded-xl bg-white/5">
                    <p className="text-xl font-bold text-white">{selectedStudent.progressPercent}%</p>
                    <p className="text-[10px] text-slate-500 uppercase">Progress</p>
                  </div>
                  <div className="p-3 rounded-xl bg-white/5">
                    <p className="text-xl font-bold text-white">{selectedStudent.averageInterviewScore}</p>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedStudent.skills.map((sk, i) => (
                        <span key={i} className="text-xs px-2 py-1 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20">{sk}</span>
                      ))}
                    </div>
                  </div>
                )}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {studentDetail.feedbacks.map((f, i) => (
                        <div key={i} className="p-2.5 rounded-lg bg-white/5 text-xs">
              ...
              {/* Feedback Form */}
              <div className="glass-card p-6">
                <h3 className="text-sm font-bold text-white mb-3">Send Feedback to {selectedStudent.name}</h3>
                <textarea
