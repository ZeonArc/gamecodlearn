  ...
  return (
    <div className="fade-in">
      ...
      <div className="glass-card p-8 mb-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-72 h-72 bg-gradient-to-br from-blue-500/10 to-violet-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {quickStats.map((stat, i) => (
          <div key={i} className="stat-card group hover:border-white/20 transition-all duration-300">
            ...
            <p className={`${stat.isText ? 'text-lg' : 'text-3xl'} font-bold text-white mt-2`}>{stat.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card p-6">
          <h2 className="text-lg font-bold text-white mb-4">Your Current Skills</h2>
          {skills.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {skills.map((skill, i) => (
                <span key={i} className="px-3 py-1.5 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/20 text-sm font-medium">
                  {skill}
                </span>
              ))}
            </div>
        ...
        <div className="glass-card p-6">
          ...
          <div className="space-y-2">
            <Link to="/profile" className="block w-full text-left nav-link hover:bg-white/10">
              <span>◉</span><span>Edit Profile & Skills</span>
            </Link>
            <Link to="/roadmap" className="block w-full text-left nav-link hover:bg-white/10">
              <span>◈</span><span>View Learning Path</span>
            </Link>
            <Link to="/interview" className="block w-full text-left nav-link hover:bg-white/10">
              <span>◎</span><span>Practice Interview</span>
            </Link>
            <Link to="/teacher" className="block w-full text-left nav-link hover:bg-white/10">
